...
class Layer:
    ...
    def linear(self):
        ...
        No return
        '''
        self.net_act = self.net_in #f(x) = x
    ...
    def softmax(self):
        ...
        No return
        '''
        max = np.max(self.net_in, axis = 1, keepdims=True)
        exp = np.exp(self.net_in - max)
        sum = np.sum(exp, axis = 1, keepdims=True)

        self.net_act = exp / sum
    ...
    def cross_entropy(self, y):
        ...
        '''
        n = self.net_act.shape[0] #Mini-batch Size
        loss = ((-1/n) * np.sum(np.log(self.net_act[np.arange(n), y]))) #Cross Entropy Loss Function
        return loss
    ...
    def forward(self, inputs):
        ...
        The net_act.
        '''
        #Saving the Input passed in
        self.input = inputs

        #We are calculating the NetIn and NetIn, just pass in the values to the functions above.
        self.compute_net_in()
        self.compute_net_act() #differentiates between activation functions
    ...
    def backward(self, d_upstream, y):
        ...
        #Gradient D_Net_In (NetAct to NetIn)
        d_net_in = self.backward_netAct_to_netIn(d_upstream, y)
        
        #Gradient DPrev_Net_Act, D_wts, D_b (NetIn to Prev Layer NetAct)
    ...
    def compute_net_in(self):
        ...
        self.net_in = (self.input @ self.wts) + self.b #Calculating Net_In operation
    ...
    def compute_net_act(self):
        ...
        implemented.
        '''
        #Part of the Forward Pass (Make sure it passes the name variable to differientate between the activation functions)
        ...
        if self.activation == 'linear':
            ...
            raise Exception("Wrong Activation Name, The name is", self.activation)
    ...
    def backward_netAct_to_netIn(self, d_upstream, y):
        ...
        #Linear Gradient
        if self.activation == 'linear': #Reshapes d_netAct -> d_netIn
            # batch_sz, n_chans, img_y, img_x = self.input.shape
            # d_netIn = d_upstream.reshape(batch_sz, n_chans, img_y, img_x)
            return d_upstream

        #ReLu Gradient
        elif self.activation == 'relu':
            d_netIn = d_upstream * (np.where(self.net_in > 0, 1, 0))
            return d_netIn
        
        #Softmax Gradient
        elif self.activation == 'softmax': 
            #calculate the y_hot variable for softmax derivative
            y_hot = self.one_hot(y, self.net_act.shape[1]) #netAct is of shape(B,C), C = # of classes in layer
            d_netIn = d_upstream * (self.net_act * (y_hot - self.net_act))
...
class Conv2D(Layer):
    ...
    def __init__(self, number, name, n_kers, ker_sz, n_chans=3, wt_scale=0.01, activation='linear', reg=0, r_seed=None,
        ...
        self.b = rng.normal(loc = 0, scale = wt_scale, size = n_kers)
...
class MaxPool2D(Layer):
    ...
    def compute_net_in(self):
        ...
        '''
        #computes net_in from max_poolnn function
    ...
    def backward_netIn_to_prevLayer_netAct(self, d_upstream):
        ...
        #Empty Array to store accumulated results of d_upstream values
        dprev_net_act = np.zeros(shape=(mini_batch_sz, n_chans, img_y, img_x)) #Is of shape self.input

        #We have access to self.pool_size and self.strides for calculating the window of self.input from d_upstream
        for b in range(mini_batch_sz_d):
            for n in range(n_chans_d):
                for row in range(out_y): #Loops through d_upstream rows
                    for col in range(out_x): #Loops through d_upstream cols
                        #Here we calculate the pool window on self.input
                        input_i = row * self.strides
                        input_j = col * self.strides
                        y_range = input_i + self.pool_size
                        x_range = input_j + self.pool_size

                        #We grab the linear index of the most active input in self.input
                        max_linear = np.argmax(self.input[b,n, input_i:y_range, input_j: x_range])

                        #Convert the linear index into linear slices
                        max_y, max_x = self.ind2sub(max_linear, (self.pool_size, self.pool_size))

                        #Assigning d_upstream to prev_net_act
                        dprev_net_act[b,n,input_i + max_y, input_j + max_x] += d_upstream[b,n,row, col]
...
class Flatten(Layer):
    ...
    def backward_netIn_to_prevLayer_netAct(self, d_upstream):
        ...
        d_wts = None
        d_b = None
        dprev_netAct = d_upstream.reshape(batch_sz, n_chans, img_y, img_x)

        return dprev_netAct, d_wts, d_b
...
class Dense(Layer):
    ...
    def __init__(self, number, name, units, n_units_prev_layer, wt_scale=0.01, activation='linear', reg=0, r_seed=None,
        ...
        self.b = rng.normal(loc = 0, scale = wt_scale, size = units)
    ...
    def backward_netIn_to_prevLayer_netAct(self, d_upstream):
        ...
        #d_upstream acts as d_netIn
        dprev_netAct = (d_upstream @ self.wts.T)
...
class Dropout(Layer):
    ...
    def __init__(self, number, name, rate=0.5, r_seed=None, verbose=True):
        ...
        self.rate = rate
        self.mode = False #Initially false for training mode
        self.mask #Used in backward function

        #Initilializing random number generator
    ...
    def compute_net_in(self):
        ...
        #Mode Condition Test
        if self.mode == False: #Meaning mode is in test mode
            self.net_in = self.input


        else:
            #Creating an array of randomly generated values
            rand_arr = self.rng.random(size=self.input.shape)

            #Using the random array to generate the mask array
            #As well as applying the mask to the input to create the net_in
            mask = np.where(rand_arr > self.rate, 1, 0)
            net_In = (self.input * mask) / (1 - self.rate) #Scaling as we apply the mask

            #Saving the mask and netIn
            self.mask = mask
            self.net_in = net_In
